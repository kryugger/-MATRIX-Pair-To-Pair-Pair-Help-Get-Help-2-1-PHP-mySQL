...
<?php
$data = mysql_fetch_array(mysql_query("SELECT reg, hr, rmv, refbonus, veri, iptm FROM general_setting WHERE id='1'"));


$myip = $_SERVER['REMOTE_ADDR'];
$timeDelay = $data[5]*3600;
$lastReg = mysql_fetch_array(mysql_query("SELECT regtm FROM users WHERE regip='".$myip."' ORDER BY id DESC"));
$canreg = $lastReg[0]+$timeDelay;




if ($data[0]==1 && $canreg<time()) {


if (isset($_GET['ref'])) {
$refname = $_GET['ref'];
$rid = mysql_fetch_array(mysql_query("SELECT id FROM users WHERE username='".$refname."'"));
$ref = (int) $rid[0];
}else{
  $ref = 0;
}
...
$capv = mysql_real_escape_string($_POST["capv"]);
...
$plan2 = mysql_fetch_array(mysql_query("SELECT st FROM packs WHERE id='".$plan."'"));
if($plan2[0]!=1){
$err14=1;
}
...
$res = mysql_query("INSERT INTO users SET username='".$username."', password='".$passmd."', fname='".$firstname."', lname='".$lastname."', phone='".$phone."', email='".$email."', bank_name='".$bname."', bank_acname='".$acname."', bank_acno='".$acno."',  regtm='".time()."', ref='".$ref."', block='0', verify='".$data[4]."', verifycode='".$verifycode."',   regip='".$myip."'");
...
abiremail2($email, "Registration Completed Successfully", $namess, $txt);
...
Already Have An Account? <a href="<?php echo $baseurl; ?>/login"> Login Now </a> 
</h4>
...
<?php 
if ($data[0]==0) {
echo '<h3 style="text-align: center; text-transform: uppercase; color:red;">NEW REGISTRATION IS OFF NOW<br>PLEASE CHECK BACK LATER</h3>';
}


if ($canreg>time()) {
echo '<h3 style="text-align: center; text-transform: uppercase; color:red;">IT SEEMS YOU ALREADY HAVE AN ACCOUNT WITH US</h3>';
}
...
<div class="col-md-3">
<img src="<?php echo $baseurl; ?>/captcha.php?code=<?php echo $captcha; ?>" alt="*" style='width: 100%;'>
</div>
...
<input type="text" name="capv" class="form-control input-lg" required="">
...
<p style="font-size: 18px;">
    ...
    <i></i>
    I have read agreed with the <a href="<?php echo $baseurl; ?>/Terms">Terms of Use</a> , 
<a href="<?php echo $baseurl; ?>/PrivacyPolicy">Privacy Policy</a> &amp; 
<a href="<?php echo $baseurl; ?>/Warning">WARNING</a> 
    </label>
...
<button class="button button-3d btn-block nomargin" id="register-form-submit" name="register-form-submit" value="register">Register Now</button>
</div>
...
var phone = $("#phone").val();
$.post( 
       "<?php echo $baseurl; ?>/jsapi-phone.php",
        { 
        phone : phone
          },
		function(data) {
